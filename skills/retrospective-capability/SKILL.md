...
本技能还负责复盘文件爆炸控制：显式复盘请求默认落文件，但不是“一次信号一个档案”。复盘文件只承接有长期学习价值的标准 / 深度复盘；同类轻量信号优先聚合到一篇专题复盘，原始 Issue、事故、报告、会议、log、ledger、handoff 和 TASK 继续留在各自 owner 页面。

复盘档案默认使用年度物理分区 + 多维索引：正文进入 `projects/retrospectives/<year>/`，完整时间索引进入 [[projects/retrospectives/indexes/by-year]]，主题索引进入 [[projects/retrospectives/indexes/by-theme]]，类型索引进入 [[projects/retrospectives/indexes/by-type]]。根 [[projects/retrospectives/README]] 只保留入口、规则摘要、索引链接和维护说明。
...
- `sensor`：`python3 scripts/check_all.py --only retrospective-system,skill-maturity` 检查总 skill、子 skill、模板字段、archive root、年份目录、索引接线和单一信息源边界。
...
- 显式“复盘 / 写复盘 / 沉淀复盘 / 总结教训”：默认标准复盘并落档到 `projects/retrospectives/<year>/`，除非用户明确说只口头、不写文件或先分析。
- 显式“深度 / 完整 / 全面复盘 / 为什么没有自动做好 / 举一反三”：进入深度复盘；首轮目标、用户纠偏序列、原始 session / rollout 证据计划、git / diff / 检查输出、产物即档案和上层抽象都必须纳入。
...
- **档案存放**：标准 / 深度复盘正文进入对应年份目录，并同步 [[projects/retrospectives/indexes/by-year]]；如有稳定主题或类型，同步主题 / 类型索引。
- **文件爆炸控制**：先判断是单篇档案、聚合专题、轻量 checkpoint 还是 owner 页补记；禁止为每个纠偏、Issue、agent episode 或检查失败预建独立复盘文件。
...
3. 用 [[templates/project-retrospective-template]] 在 `projects/retrospectives/<year>/` 建立复盘档案；轻量 checkpoint 可以只写到 [[log]]、Closeout Proof 或 owner 页。
4. 还原目标、实际结果、关键事实和偏差原因；每个判断标注 `confirmed / likely / possible / blocked`。
5. 先做上轮行动兑现回检，再写本轮改进行动。
6. 把行动分流到已有 owner 页面；不要在复盘目录形成平行看板。
7. 同步 [[projects/retrospectives/indexes/by-year]]；如果属于稳定主题或类型，同步 [[projects/retrospectives/indexes/by-theme]] 或 [[projects/retrospectives/indexes/by-type]]。
8. 按治理自演进判断决定是否更新模板、skill、sensor、[[WORKFLOW]]、[[AGENTS]]、[[POLICY]] 或 [[harness-feedback-ledger]]。
9. 跑 `python3 scripts/check_all.py --only retrospective-system`，收尾前跑完整门禁。
...
- 不把复盘正文平铺到 `projects/retrospectives/` 根目录，或把主题 / 类型误做成物理目录。
- 不把默认落文件理解成一信号一档案，导致复盘文件爆炸。
