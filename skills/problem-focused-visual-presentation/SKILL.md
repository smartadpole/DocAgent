...
当核心信息是“行列交叉状态”，例如技能 x 工程、功能 x 环境、风险 x owner、用例 x 验证层级、证据 x 结论时，矩阵本身就是概览，不要把它默认下沉为详情附录。

矩阵 / 热力图的默认视觉原则：

- 概览矩阵优先前置，先让用户一眼看到分布、断点、密集区和异常点；具体解释放到矩阵后面的详情卡、脚注或追溯区。
- 状态单元格优先用整格填色表达状态，而不是在白底单元格里放胶囊、标签或小徽章；胶囊适合图例和卡片摘要，不适合承担矩阵主视觉。
- 不同状态之间要有足够的色相、明度或饱和度差异，让用户扫视时能看到结构性的“碰撞”，而不是一片温和但难分辨的装饰色。
- 单元格文字必须和背景形成强对比；必要时使用反白、加粗、轻微阴影或更深 / 更浅的背景，而不是为了柔和牺牲可读性。
- 色彩服务语义，不绑定具体配色；目标是让状态强弱、成熟 / 缺口、通过 / 阻塞、源 / 下游等差异在第一眼成立。
- 详细说明、路径、证据和长文本不要塞进矩阵格子；矩阵格子只承载短标签、符号或数值，详情另设卡片或展开区。
- 如果矩阵是用户明确要检查的主对象，不能用摘要卡、优先子集或折叠区替代它；摘要只能解释矩阵，不能把主矩阵藏起来。
- 不要在同一页面重复渲染同一张矩阵。主次分层优先靠行列标记、排序、筛选提示、单元格链接和详情页完成；只有两个矩阵回答不同问题时，才允许并存。
- 标签必须有区分力。如果一个 scope 里的所有行都同属“优先”“待看”或同一类别，就把这个事实放到分组说明或图例里，不要逐行重复。
- 矩阵密度也是质量指标。状态、分数和短符号优先同格或同排呈现，行高、列宽、首列说明和边框都应服务扫读，避免把主视图做成大色块堆叠。
- 折叠区只承接详情、诊断、证据、方法说明或次级摘要；不要把用户最关心的核心对象折叠掉，再让用户自己找回来。
- 导出 / 打印时保留整格色块，不退化成只有文字或只有浅边框的表格。
