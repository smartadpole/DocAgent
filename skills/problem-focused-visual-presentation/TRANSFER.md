...
- 导出能力：HTML print view、PDF export、A4 / A3 / custom 页面规格、横竖版选择、分页策略、页眉页脚、证据边界和 snapshot 规则。
...
4. 如果目标工程有专题目录或视图目录，建立 current / snapshot / exports 或等价呈现层；没有则先只输出聊天图文 lens。
...
- 用一个 HTML 导出样本验证：输出必须声明 A4 / A3 或 custom、横竖版、边距、分页策略，并在实际导出 PDF 时检查图表裁切、页数、页脚来源和打印可读性。
