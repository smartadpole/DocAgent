...
如果本技能由 [[skills/technology-research-router/SKILL]] 路由进入，必须继承 router 的对象类型、决策目标、证据来源计划、成熟度假设、风险门和沉淀路径，不重新把问题退回“介绍一下这个技术”。
...
- 本轮对象确实是技术概念、技术路线、架构方法或算法机制。
- 本轮结论要服务学习、选型、PoC、替换、采购、产品化还是知识储备。
- 近期事实是否需要查证：版本、价格、license、benchmark、论文、产品化状态、开源活跃度。
- 哪些风险可能阻断采用：数据、算力、隐私、安全、合规、供应链、维护成本。
- 最终要落到 article / concept / template / skill / project 的哪一层。
...
生态扫描必须标明证据等级：一手事实、权威分析、产业信号、媒体线索、推论或建议。不能把宣传、二级文章或模型回答直接写成事实。
...
PoC 结论要有退出条件：指标不达标、成本不可控、无法复现、数据要求过高、治理风险不可接受，都可以成为停止理由。
...
结论必须同时给成熟度和行动等级，例如 `TRL / 工程成熟度 + A/B/C/D`，避免把“前沿”“热门”“有潜力”误写成“现在该采用”。
...
- 是否给出 PoC、风险和投入等级。
- 是否区分已验证事实、推论和待确认项。
- 是否给出成熟度、行动等级、阻断风险和刷新触发条件。
- 是否说明本轮调研继承了总控 intake，还是本轮直接进入分支技能。
