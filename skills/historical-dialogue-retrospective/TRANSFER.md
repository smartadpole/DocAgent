...
当用户要求“通用版”“直接做通用的”“不要特定工程定制”时，复盘迁移提示词必须使用通用版：

- 保留上面的完整任务书骨架和执行粒度。
- 不出现具体目标工程小节。
- 不列具体目标工程的路径读取清单。
- 不写 `prefect`、`customeranalysis` 或其他当前上下文里的仓库级落位建议。
- 把落位建议写成结构自检条件：有 `projects/` 怎么办，没有 `projects/` 怎么办，已有 retrospective / postmortem / incidents / handoff / templates 时怎么判断。
- 如果上一版提示词已经包含具体工程定制，改通用版时只删除定制段落，保留通用主干，不重新压缩。

如果用户明确要求“按多个目标工程分别定制”，最终提示词才在完整通用任务书之后追加每个目标工程的差异化落位说明。不要把多工程差异提前写成主叙事，导致“复盘体系目标、参考资料、模块清单、入口同步、最终交付”被压缩。差异化说明重点写结构事实、应复用入口、禁止误用目录、需要覆盖的默认落位和各仓库的检查 / 提交边界。

## 优于示例的判定标准

当用户给出复盘迁移提示词示例并要求生成稿更好时，生成稿必须先完整覆盖示例，再补强。判定如下：

- **不能少**：开头目标、复盘体系目标、参考资料、吸收边界、目标工程结构自检、八类复盘体系模块、入口同步、最终交付和最终回复要求。
