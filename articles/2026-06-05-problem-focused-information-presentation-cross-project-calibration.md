...
用户真正要的不是更漂亮的文档，也不是一次性 dashboard，而是“一图胜千言”的图文阅读层：同一个问题再次被问起时，系统能找到稳定的当前 lens，刷新真相源，更新当前判断，用表格、脑图、框图、关系图、时间线和状态卡呈现；当需要下载、打印、批注或线下流转时，提前声明 A4 / A3、横竖版、分页和 PDF / PNG 导出策略；导出形态必须和 HTML current lens 同源同版，并在关键节点保留不可变快照。
...
6. **支持可下载和可打印交付**：用户有时不是只在屏幕里看，还需要把图文 lens 导出成 PDF / PNG、A4 / A3 打印、线下批注或外部分发；因此导出版式必须在设计阶段进入方案，而不是事后截图。
7. **避免重复渲染物入库**：工程里已经有 canonical HTML lens 时，不应同时把同一视图的 PDF、PNG、SVG 放进可提交目录；导出件可以提供下载，也可以放在 gitignore 忽略目录，但不应成为提交内容。

因此，方案的中心不是“生成 HTML 文件”本身，而是：围绕稳定关注对象建立可刷新、可追溯、可归档、可视化表达充分、可按需要导出的 lens 体系。HTML 是默认承载容器；如果 HTML 排版不能充分表达，就应补充 SVG、Canvas、Mermaid、ECharts / D3、Excalidraw 图、PDF / slide、独立图片或 HTML + assets 组合包。若用户需要打印或下载，HTML lens 应优先具备 print view，并用 `export_profile` / `print_profile` / `equivalence_profile` 约束 PDF / PNG 输出。
...
7. **导出版式属于 lens 设计，不是收尾动作**：只要用户可能下载、打印、线下批注或外部分发，就要在 lens 里声明 `export_profile`、`print_profile` 和 `equivalence_profile`，包括 A4 / A3 / custom、横排 / 竖排、边距、分页、页眉页脚、重复表头、图表裁切、来源页脚、证据边界，以及 HTML / PDF / PNG 是否同源同版。PDF / PNG 是从 current lens 派生的导出物；只有 PDF 用于验收、决策、发布、事故或复盘固化且用户明确要求归档外部分发件时，才进入 snapshot 附件。

8. **导出件默认不入库**：`views/current/` 和 `views/snapshots/` 默认只提交 canonical HTML / source / manifest；PDF、PNG、SVG 预览或下载件进入 `views/exports/`、`views/.exports/`、`assets/views/` 或目标工程等价 gitignore 目录。`assets/` 只放源级支持材料、真实证据截图、原始图片或不可再生附件，不放由 HTML lens 生成的重复预览图。
...
7. **声明导出配置**：如果需要下载、打印或分发，选择 A4 / A3 / custom、横排 / 竖排、分页策略、print CSS、PDF / PNG 生成方式、忽略目录和同源一致性策略。
8. **更新 current**：覆盖 canonical lens，并记录更新时间、来源、证据边界、失效条件和导出配置。
9. **判断是否 snapshot**：只有关键裁决、验收、发布、事故、复盘或外部分发时，另存不可变快照；PDF / PNG 如果只是下载或打印副本，不自动等于 snapshot。

## 方案边界

- Markdown / 数据 / 报告仍是真相源，不被 HTML 替代。
- HTML 是默认阅读容器，不应该散落在每个源页面旁边；当 HTML 不足以表达关系时，应由配套图像、图表或演示型文件补足。
- PDF / PNG / print view 是导出和分发层，不是真相源；导出前必须检查分页、图表裁切、来源页脚、证据边界、打印可读性，以及和 HTML lens 的信息 / 版式一致性。
- 工程中不同步提交同一 lens 的 HTML、PDF、PNG、SVG；导出件放 gitignore 忽略目录或运行时下载，避免同一视图重复入库。
