...
这些工程不是目标对象，而是复杂度样本。它们共同用于校准 [[concepts/problem-focused-information-presentation]]：当用户反复关注同一个问题时，知识库最终应该怎样生成合适的 HTML lens。

## 一句话总结

用户真正要的不是更漂亮的文档，也不是一次性 dashboard，而是一个能随关注问题自动切换的阅读层：同一个问题再次被问起时，系统能找到稳定的当前 HTML lens，刷新真相源，更新当前判断，并在关键节点保留不可变快照。
...
2. **让每次阅读都服务当前判断**：看状态、行动、验收、风险、知识和资源时，展示结构应该不同。
3. **保留证据边界**：直观不等于省略证据。HTML lens 必须明确哪些结论是当前态，哪些只是历史快照、局部验证或辅助证据。
4. **形成用户视角体系**：用户入口应是“我要看什么问题”，而不是 `projects/`、`articles/`、`raw/` 等维护目录。

因此，方案的中心不是“生成 HTML 文件”本身，而是：围绕稳定关注对象建立可刷新、可追溯、可归档的 lens 体系。
...
1. **同一个稳定关注对象只能有一个当前入口**：多次问同一问题时，应先解析 `focus_object + lens_type`，查 `views/lens-registry.md`，刷新源，再更新 canonical HTML lens。不能每次散落生成一个新文件。

2. **snapshot 只在证据需要冻结时产生**：验收、决策、发布、事故、阶段复盘和外部分发应保留 snapshot。普通追问、澄清和轻量刷新只更新 canonical lens。

3. **HTML lens 必须有 freshness 判断**：视图需要显示 `generated_at`、`source_revision`、`source_pages` 和 `staleness_policy`。如果源页面、报告、服务状态或数据快照变化，旧 HTML 不能继续冒充当前态。

4. **用户入口要先于文件入口**：入口应按“看状态 / 要行动 / 要拍板 / 看风险 / 要验收 / 理解知识 / 找资源 / 复盘过程”组织，再映射到底层源页面。

5. **不同工程需要 adapter，但不能把工程规则写死到通用方案**：Life、DocCustomer、prefect、fetch-adapter、DocFilmCommunity 可以各自声明高频 lens pack；通用层只定义关注对象、证据边界、刷新流程、registry 和 HTML provenance。
...
4. **刷新当前判断**：把源里的最新事实投影成一眼判断层、证据解释层和原始追溯层。
5. **更新 HTML current**：覆盖 canonical HTML lens，并记录更新时间、来源、证据边界和失效条件。
6. **判断是否 snapshot**：只有关键裁决、验收、发布、事故或外部分发时，另存不可变快照。

## 方案边界

- Markdown / 数据 / 报告仍是真相源，不被 HTML 替代。
- HTML 是最终阅读界面，不应该散落在每个源页面旁边。
- lens 不是固定模板，而是阅读协议；简单问题可以短文本回答，复杂问题才升级为 HTML。
