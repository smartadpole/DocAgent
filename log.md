...
- **记录人**：sunhao
- **用户意图**：把飞书里的拉流资料先沉淀成文档，再设计初步验证方案，同时确认工具和看板是否能正常拉到业务单元数据。
- **主题**：
  1. 资料整理与文档沉淀。
  2. 初步测试方案设计。
  3. 本地订单中心与历史工作台实测。
- **关键动作**：
  1. **主沉淀**：新增 [[articles/2026-04-14-feishu-上游准备阶段-validation-plan]]，把 `内部服务主机`、长期 token 处理方式、已验证现状和测试矩阵整理成可回看的文档。
  2. **主实测**：在 `http://本地服务地址` 验证订单中心与历史工作台可用，确认 `北京七十二店` 当前可拉到 4 条订单级记录、31 条历史记录总量，且订单级记录都落在 `2026-03-02`。
  3. **主收口**：明确当前可验证样例是 `2026030200001-4`，用户指定的 `北京七十二店 / 58 号桌 / 19:00` 仍需补齐真实数据源后再做最终验证。
- **影响页面**：[[articles/2026-04-14-feishu-上游准备阶段-validation-plan]]、[[log]]。

### 对话：纠正样例里的业务记录编号表述

- **记录人**：sunhao
- **用户意图**：纠正前一轮文档里关于样例字段的表述，避免把业务记录编号误写成原始业务字段。
- **主题**：
  1. 收紧样例说明里的业务记录编号表述。
  2. 明确业务记录编号和桌号、时间点的关系。
- **关键动作**：
  1. **主修正**：把 [[articles/2026-04-14-feishu-上游准备阶段-validation-plan]] 里的样例说明改成“业务记录编号只是回溯和定位样例的键，不是原始业务字段”。
  2. **主澄清**：明确真正要核对的是桌号、时间点和业务单元，业务记录编号只是用来反推对应记录。
- **影响页面**：[[articles/2026-04-14-feishu-上游准备阶段-validation-plan]]、[[log]]。

## 2026-04-13
### 对话：确认会议记录默认是一会一条，预读材料则单独成页
...
## 2026-04-13
### 对话：把 commit message 的语言规则写进维护约束
