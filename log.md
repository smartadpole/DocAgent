...
### 对话：完善 `[[log]]` 规则、模板和记录方式，避免重复拆条

- 记录人：sunhao
- 用户意图：持续收紧 `[[log]]` 的规则、模板和历史写法，让记录既完整又不重复；凡是仍属于同一条日志治理意图的后续纠偏、命名收口、格式统一和规则补充，都尽量并回已有记录，而不是不断拆成新条。
- 主题：
  1. 为 `[[log]]` 增加记录人字段，并把来源收口到 `git user.name`。
  2. 把 `[[log]]` 的完整记录规则迁出历史正文，收口到 [[log-writing-rules]]。
  3. 补全 `[[log]]` 的合并、拆解、延续、重开、回填边界。
  4. 收紧 `[[log]]` 的编号模式、模板单一信息源和内部链接规则。
  5. 明确“最近这些 `[[log]]` 治理动作其实属于同一条记录，应优先完善旧记录而不是继续拆条”。
- 关键动作：
  1. 确认当前仓库的 `git user.name` 为 `sunhao`，适合作为 `[[log]]` 中“记录人”字段的默认来源。
  2. 在 [[log-writing-rules]] 中补充可选字段规则，明确记录人优先写 `git user.name`；如果仓库没有配置，就留空，不回退到本机用户名。
  3. 在 [[templates/log-entry-template]] 中加入可选的“记录人”字段，并补充使用说明，要求如果要写记录人，就优先使用当前仓库的 `git user.name`。
  4. 随后继续把 [[log-writing-rules]] 中默认骨架里的字段名从“记录人（可选）”收成“记录人”，同时保留“需要标注时优先写 `git user.name`；没有配置就留空”的解释。
  5. 将 [[templates/log-entry-template]] 中的字段名同步改为“记录人”，不再把“可选”写进模板正文。
  6. 将这条样例记录同步收成“记录人：sunhao”，保持规则、模板和实例一致。
  7. 在 [[AGENTS]] 中新增“批处理模式”，在 [[WORKFLOW]] 中新增“轻量批处理模式”，为批量文章摘要和批量设计整理收口一条低成本但不降质的处理路径。
  8. 新建 [[log-writing-rules]]，把 `[[log]]` 的记录单位、合并与拆解标准、记录级 / 主题级判断，以及延续 / 重开 / 回填边界统一迁过去，作为唯一主入口。
  9. 把 `[[log]]` 顶部从完整规则正文收短成历史页说明，只保留对 [[log-writing-rules]] 和 [[templates/log-entry-template]] 的跳转；同步收短 [[WORKFLOW]] 和模板中的重复展开，并把 [[README]]、[[INDEX]] 的入口关系补到新规则页上。
  10. 在 [[log-writing-rules]] 中把合并 / 拆解标准从隐含口径补成显式规则，明确优先看用户意图、主结果、因果链和回看成本，并区分“记录级合并”和“主题级合并”两层判断。
  11. 在 [[log-writing-rules]]、[[WORKFLOW]] 和模板中补齐“延续 / 重开 / 回填”边界，明确跨日期的同主题默认新开当天记录；只有日期纠偏或补记遗漏时才回填旧日期。
  12. 将 2026-04-12 内分散的历史回填、日期纠偏、旧记录整合等治理动作合并收口，避免同一天内部再次出现同类治理动作的重复记录。
  13. 把前几天真实发生的修正回填到原始日期，而不是继续误记在 2026-04-12 名下，同时整合错位、重复和分散的旧记录。
  14. 把主题与关键动作的编号列表模式正式提升为记录规则，而不再只是模板示例；并把 `[[log]]` 顶部关于模板入口的重复说明收成一句。
  15. 追查最近几轮实际有变更却没同步写入 `[[log]]` 的执行遗漏，并在 [[AGENTS]] 与 [[WORKFLOW]] 中补成更明确的硬约束：只要这次对话产生了实际内容或结构变更，就必须同步更新 `[[log]]`。
  16. 把 `[[log]]`、[[WORKFLOW]] 和 [[templates/log-entry-template]] 中的“主题”和“关键动作”统一收成编号列表，并把已有历史条目迁移到同一格式。
  17. 删除 `[[log]]` 和 [[WORKFLOW]] 中重复展开的模板正文，只保留规则说明和对 [[templates/log-entry-template]] 的跳转；同时把模板入口中的纯文本文件名改成可点击的 `[[wikilink]]`。
  18. 在 [[AGENTS]] 和 [[WORKFLOW]] 中新增专项检查，明确禁止复制第二份模板正文，并要求凡是语义上在引用本库页面、模板页、入口页或跳转目标，都必须写成 `[[wikilink]]`。
  19. 最后回看最近两条关于记录人字段的 `[[log]]` 记录，确认它们本质上属于同一条日志治理意图，于是将原本被错误拆开的“增加记录人字段”和“去掉字段里的可选字样”重新合并为同一条对话记录。
- 影响页面：[[log]]、[[log-writing-rules]]、[[templates/log-entry-template]]、[[WORKFLOW]]、[[README]]、[[INDEX]]、[[AGENTS]]、[[BRAIN]]、[[POLICY]]。
