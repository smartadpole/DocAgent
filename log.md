...
- **记录人**：sunhao
- **用户意图**：用户指出矩阵中“源头”和“领先”不应继续用第一列文字重复表述；领先已经由颜色表达，源头也应改用边框、纹理等视觉编码，并通过更细的图例解释。
- **主题**：
  1. 将源头信息从第一列冗余文字改为格子级视觉标记，避免 Goal Contract 等多源头能力撑高第一列。
  2. 保持成熟度和源头两套语义分离：底色表示成熟度，深色内边框和底部斜纹表示源头工程。
  3. 通过图例解释视觉语义，把可读说明集中到图例和边界说明里。
- **关键动作**：
  1. 更新 `scripts/update_skill_maturity_matrix.py`，为源头工程格子追加 `heat-source` 样式，并让第一列只显示能力名和子项数量。
  2. 重新生成 [[views/current/governance/skill-maturity-matrix.html]]，把图例拆成“成熟度颜色”和“源头标记”，并更新首屏说明、矩阵说明和证据边界。
  3. 通过 Browser 本地渲染检查桌面和窄屏视图，确认 Goal Contract 第一列不再显示 `源头 / 领先` 文本，源头格保留边框和底部斜纹，无控制台 warning / error。
- **结论**：矩阵总览层后续应优先用视觉编码承载重复属性；文字只保留能力名、必要数量和图例解释，详情卡再展开来源、发现工程和子项证据。
- **影响页面**：`scripts/update_skill_maturity_matrix.py`、[[views/current/governance/skill-maturity-matrix.html]]、[[log]]。

### 将技能矩阵校准为主题大项和源头视图

- **记录人**：sunhao
- **用户意图**：用户指出矩阵应仔细分析所有工程的不同主题技能；复盘、调研这类能力应作为大技能汇总，不应在总表里拆成多个细项；表格还必须标注源头，而不是只显示成熟度差异。
- **主题**：
  1. 将总表从“每个 `SKILL.md` 一行”校准为“主题大项一行、底层子项在详情展开”，避免调研、复盘等能力被拆散后失去总览价值。
  2. 保持底层扫描全量，不丢掉各工程不同主题技能；大项只改变展示粒度，不删除子项证据。
  3. 在矩阵第一列和详情卡显式显示源头工程、发现工程、子项数量和子项名称，让读者能区分“源头”“领先”和“成熟度差异”。
- **关键动作**：
  1. 更新 `scripts/update_skill_maturity_matrix.py`，新增主题归并层，把调研、复盘、项目上下文入口、文档与 Agent 规则维护、LifeOS 管理等细项归入大项，同时保留全部底层 `SKILL.md` 扫描结果。
  2. 重新生成 [[views/current/governance/skill-maturity-matrix.html]]，首屏显示汇总大项数和底层细项数，矩阵第一列显示 `源头 / 领先 / 子项`，详情卡显示源头工程、发现工程、子项列表和证据路径。
  3. 同步 [[views/README]] 和 [[views/lens-registry]]，把该 lens 的口径改成“主题归并后的源头、子项、成熟度排名和反哺候选”。
- **结论**：这张表的主职责是梳理跨工程技能主题和源头，不是把所有细 skill 平铺成清单；后续需要展开某个大项时，再用文章或专题 lens 展开该大项下的子技能、源头演进和迁移策略。
...
- **主题**：
  ...
  2. 用科博会、北京日报 / 北京市科委转发、CR 福祉展、专利、软著、Plug and Play Demo Day、中国科协活动、2026 年前沿科技大赛、海淀 AI 迎春夜和 Tech Show 材料交叉核验企业基本画像、产品路线和公开进展。
  3. 把医疗器械注册、临床验证、真实商业化、数据隐私和技术指标列为待核验边界，避免把展会、演示和路演露出上推成成熟产品证据。
- **关键动作**：
  1. 新增并补充 [[articles/2026-06-09-xinzhi-ruisheng-company-research]]，记录企业画像、可穿戴智能人工喉技术路线、公开进展、机会判断、风险边界和后续尽调清单；后续补入 2026 年前沿科技大赛、海淀 AI 迎春夜、Tech Show 以及“2026 送检 / 临床 / 取证、2027 推向市场”的更新节奏。
  2. 新增并补充 [[concepts/beijing-xinzhi-ruisheng]]，作为企业实体入口页，明确其和 [[concepts/ai-era-information-presentation]] 的关系，并同步最新阶段判断。
