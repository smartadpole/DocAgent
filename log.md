...
- **关键判断**：
  ...
  3. 正确最低形态应是首次接触定位、协作承诺、首次接触方向和推荐第一步；其中定位必须帮助用户设定系统角色、用户目标、协作方式和第一步成果。
- **关键动作**：
  ...
  10. 新任务 `019f8e86-943e-74e1-9c67-0bca7d4ca92a` 首轮“你好”经 `codex_app.read_thread` 读回完整 final answer，已包含首段协作维护者定位、`可以从这几个方向开始：`、`1.` 到 `4.` 编号方向和 `推荐第一步：这轮想解决什么`；`wait_threads` 摘要会压平 Markdown，不作为格式判定源。
- **验证 / 边界**：新任务烟测证明当前 wiki 项目冷启动在新任务中已经能输出首次接触 + 常规引导融合入口；不证明已经打开的旧会话会热更新。
...
- **关键动作**：
  ...
  5. 新增 [[templates/agent-intelligence-evaluation-template]] 并接入 `agent-system-maturity` sensor，把 positive / negative behavior corpus、八维 intelligence lens、evaluator provenance、Goodhart guard 和 external readback 边界固定成行为智能证明路径。
  6. 同步 [[AGENTS]]、[[.codex/AGENTS]]、[[INDEX]]、[[skills/README]]、[[templates/README]]、[[agent-system-maturity]]、[[agent-system-cross-project-alignment.v1]]、[[projects/README]]、[[projects/status]]、[[projects/memory/README]] 和 [[projects/memory/shared]] 的入口、状态与长期背景。
...
- **影响页面**：[[projects/design/topics/implementation-engineering-template-system]]、[[projects/design/topics/agent-workflow-memory-harness-skill-landing]]、[[templates/implementation-project-profile-template]]、[[templates/agent-intelligence-evaluation-template]]、[[projects/design/topics/README]]、[[AGENTS]]、[[.codex/AGENTS]]、[[INDEX]]、[[skills/README]]、[[templates/README]]、[[agent-system-maturity]]、[[agent-system-cross-project-alignment.v1]]、[[projects/README]]、[[projects/status]]、[[projects/memory/README]]、[[projects/memory/shared]]、[[log]]、`scripts/check_implementation_template_system.py`、`scripts/check_all.py`、`scripts/check_agent_system_maturity.py`。
