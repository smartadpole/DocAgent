...
- **关键动作**：
  ...
  6. **新增 sensor**：新增并扩展 `scripts/check_work_item_matrix.py`，接入 `scripts/check_all.py`，让事项矩阵、模板、入口和子工程回传 wiring 可检查；后续又从关键词检查升级为文件、章节、表头、模板字段和入口链接检查。
- **二阶反思**：这轮说明“完整吸收”不能只靠口头记忆；高价值工程设计必须同时落到模型、模板、入口、治理规则、子工程协作契约和本地 sensor，后续才能在新任务中自然执行。对于分布式规则，必须给维护者一个日常入口顺序；对于 sensor，优先检查结构和字段，不继续堆脆弱关键词。
