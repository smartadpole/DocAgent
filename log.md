...
- 记录人：sunhao
- 用户意图：确认 `[[log]]` 是否可以标注“谁记录的”，并把字段来源收口到更稳定的协作身份，而不是本机用户名。
- 主题：
  1. 为 `[[log]]` 增加可选的记录人字段。
  2. 将记录人字段的来源收口到 `git user.name`。
- 关键动作：
...
- 影响页面：[[log]]、[[log-writing-rules]]、[[templates/log-entry-template]]。

### 对话：去掉 `记录人` 字段里的“可选”字样

- 用户意图：保留 `记录人` 字段可省略的语义，但不把“可选”两个字直接写进日志正文和模板字段名里，避免字段视觉上变重。
- 主题：
  1. 收短 `记录人` 字段的显示名称。
  2. 把“是否必填”的含义留在规则解释里，而不是放进字段名。
- 关键动作：
  1. 将 [[log-writing-rules]] 中默认骨架里的字段名从“记录人（可选）”收成“记录人”，同时保留“需要标注时优先写 `git user.name`；没有配置就留空”的解释。
  2. 将 [[templates/log-entry-template]] 中的字段名同步改为“记录人”，不再把“可选”写进模板正文。
  3. 将 `[[log]]` 中这条关于记录人字段的样例记录同步收成“记录人：sunhao”，保持规则、模板和实例一致。
- 影响页面：[[log]]、[[log-writing-rules]]、[[templates/log-entry-template]]。

### 对话：给批量文章和设计整理增加轻量批处理模式

- 用户意图：确认当前规则和流程在批量处理技术文章、设计方案时会不会造成 token 线性放大，并把高频重复读取的成本收下来，但不牺牲规则、结构和项目判断的严谨性。
- 主题：
  1. 下游处理当前规则里导致批量处理放大 token 成本的重复读取点。
  2. 为批量文章摘要和批量设计整理补一条可执行的轻量路径。
  3. 保持重判断场景仍然走默认重路径，避免把减载误写成降质。
- 关键动作：
  1. 回看 [[README]]、[[INDEX]]、[[WORKFLOW]]、[[BRAIN]]、[[POLICY]]、[[projects/README]]、[[projects/STRUCTURE]] 和 [[AGENTS]]，确认当前高成本主要来自默认必读集过大，以及批量任务里反复支付同一组入口页的读取成本。
  2. 在 [[AGENTS]] 中新增“批处理模式”，明确只有在不改规则、不改结构、不改项目阶段、不改项目级状态、不拍板冲突时，才允许先做一次全局校准并在同批材料上复用。
  3. 在 [[AGENTS]] 的读取顺序和最小读取集中补入批处理模式专用口径，分别给出批量 `articles/`、批量设计材料和批量 `concepts/` 的共享读取方式与升级条件。
  4. 在 [[WORKFLOW]] 中新增“轻量批处理模式”，把适用范围、不适用范围、执行顺序和两类典型共享读取路径写成正式流程，明确一旦触发规则、结构、状态、取舍或记忆路由变化，就立即升级回默认流程。
- 影响页面：[[AGENTS]]、[[WORKFLOW]]、[[log]]。

### 对话：把 `[[log]]` 的完整记录原则迁出历史正文

- 用户意图：确认 `[[log]]` 不应该继续同时承担历史记录和规则说明两种职责，并按单一信息源原则把完整写法迁到独立主页面。
- 主题：
  1. 让 `[[log]]` 只保留历史记录职责。
  2. 把 `[[log]]` 写法收口成独立规则页。
  3. 收紧入口关系，避免新规则页变成孤岛。
- 关键动作：
  1. 新建 [[log-writing-rules]]，把 `[[log]]` 的记录单位、合并与拆解标准、记录级 / 主题级判断，以及延续 / 重开 / 回填边界统一迁过去，作为唯一主入口。
  2. 把 `[[log]]` 顶部从完整规则正文收短成历史页说明，只保留对 [[log-writing-rules]] 和 [[templates/log-entry-template]] 的跳转。
  3. 同步收短 [[WORKFLOW]] 和 [[templates/log-entry-template]] 中重复展开的 `[[log]]` 规则，并把 [[README]]、[[INDEX]] 的入口关系补到新规则页上。
- 影响页面：[[log]]、[[log-writing-rules]]、[[WORKFLOW]]、[[templates/log-entry-template]]、[[README]]、[[INDEX]]。

### 对话：补全 `[[log]]` 主题合并与拆解的完整判定标准

- 用户意图：确认前几轮讨论出的 `[[log]]` 合并 / 拆解标准是否已经真正落入规则，而不是只停留在对话解释里；把“按用户意图判断”“记录级 vs 主题级”“四维判定法”“延续 / 重开 / 回填”补成正式规则。
- 主题：
  1. 把 `[[log]]` 的合并与拆解标准从隐含口径补成显式规则。
  2. 区分记录级合并、主题级合并，以及跨日期时的延续 / 重开 / 回填。
  3. 把判定标准同步到模板，避免规则和使用骨架脱节。
- 关键动作：
  1. 在 `[[log]]` 的记录原则里补入核心标准，明确合并与拆解优先看用户意图、主结果、因果链和回看成本，而不是问题个数、改动文件个数或日期是否相同。
  2. 把“记录级合并”和“主题级合并”明确成两层不同判断，避免把“是否是一条对话记录”和“同一条记录下面写几个主题”混成一层。
  3. 在 `[[log]]` 和 [[WORKFLOW]] 中补齐“延续 / 重开 / 回填”边界：跨日期的同主题默认写新记录并保留延续关系；只有日期纠偏或补记遗漏时才回填；如果已经形成新的独立判断任务，就按重开处理。
  4. 在 [[templates/log-entry-template]] 中同步补入这套使用说明，确保后续套模板时也会按同一标准落地。
- 影响页面：[[log]]、[[WORKFLOW]]、[[templates/log-entry-template]]。

### 对话：明确 `[[log]]` 的跨日期合并边界，并按新边界重组当天记录

- 用户意图：澄清“同一主题是否要跨日期继续并回旧记录”的边界，避免“同主题应合并”和“日志按日期 / 按对话维护”两套规则互相打架；同时把 2026-04-12 当天已经写下的相关记录按新边界重组，形成可直接执行的样例。
- 主题：
  1. 明确 `[[log]]` 的合并边界是“单次对话”，不是“主题的整个历史生命周期”。
  2. 明确跨日期的同主题默认写新记录，只有纠偏或补记遗漏时才回填旧日期。
  3. 按这套边界重组 2026-04-12 当天的历史回填类记录。
- 关键动作：
  1. 明确“记录内合并”和“历史间关联”是两层不同动作：同一轮对话内的相关问题可以合并，但跨天的同主题默认只保留延续关系，不直接回并旧记录正文。
  2. 在 `[[log]]` 和 [[WORKFLOW]] 的记录规则里补入跨日期边界，要求跨日期再次处理同一主题时默认新开当天记录；只有在纠正历史归属日期，或补记本来就属于当时那轮对话但漏写的内容时，才回填旧日期。
  3. 将 2026-04-12 里两条分散的“历史回填 / 日期纠偏 / 旧记录整合”条目收成一条，避免同一天内部再次出现同类治理动作的重复记录。
- 影响页面：[[log]]、[[WORKFLOW]]。

### 对话：回填前几天的真实变更到原始日期，并整合旧记录

- 用户意图：把前几天已经发生的实际修正回填到真实日期，而不是误记到 2026-04-12；同时整合同类历史条目，避免同一主题在不同日期里错位、重复或分散。
- 主题：
  1. 把前几天已经发生的实际变更回填到原始日期。
  2. 整合旧记录，避免同一主题在不同日期里错位和重复。
  3. 区分“今天在整理历史”与“今天实际新增了什么内容”。
- 关键动作：
  1. 回看前几天围绕内部页面引用规范、wikilink 渲染异常、README 首章结构统一，以及“最小改造方案”补齐过程的历史链路，确认哪些动作本来就发生在 2026-04-09 和 2026-04-10。
  2. 把这些实际发生在前几天的内容分别补回原始日期下，作为当时连续推进的一部分，而不是继续挂在 2026-04-12 名下。
  3. 同步整合 2026-04-10 已有的相关条目，避免同一主题既留在原日期里，又在 2026-04-12 再重复一遍。
  4. 保留 2026-04-12 的记录，但只说明今天做了历史回填和整理动作，不把前几天的真实修正误记到今天。
- 影响页面：[[log]]。
