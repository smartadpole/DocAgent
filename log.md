...
- **记录人**：sunhao
- **用户意图**：纠正前一次反哺时对“可复用系统层信息”的定义过窄问题，明确只要是规则就默认需要反哺，并把 DocFilmCommunity 中遗漏的规则、流程、写法和记忆路由抽象回模板库。
- **主题**：
  1. 重新定义模板反哺的系统层范围。
  2. 区分可复用系统信息和具体项目材料。
  3. 补齐 DocFilmCommunity 中已经验证的规则类反哺。
- **关键动作**：
  1. **主规则**：更新 [[template-feedback-rules]]，把可反哺系统层信息扩展为结构、流程、规则、记忆系统、写法格式、模板、自动化契约和治理说明，并明确“只要它是规则，就默认需要反哺”。
  ...
  5. **主入口**：同步更新 [[README]]、[[INDEX]] 和 [[governance/README]] 中的模板反哺入口描述，避免入口页继续把反哺范围收窄成结构和模板。
- **影响页面**：
...
## 2026-04-14
### 把拉流资料沉淀成文档，并在本地订单中心做初步实测

- **记录人**：sunhao
- **用户意图**：把飞书里的拉流资料先沉淀成文档，再设计初步验证方案，同时确认工具和看板是否能正常拉到门店数据。
- **主题**：
  1. 资料整理与文档沉淀。
  2. 初步测试方案设计。
  3. 本地订单中心与历史工作台实测。
- **关键动作**：
  1. **主沉淀**：新增 [[articles/2026-04-14-feishu-stream-pull-validation-plan]]，把 `reflow-host`、长期 token 处理方式、已验证现状和测试矩阵整理成可回看的文档。
  2. **主实测**：在 `http://127.0.0.1:5000` 验证订单中心与历史工作台可用，确认 `北京七十二店` 当前可拉到 4 条订单级记录、31 条历史记录总量，且订单级记录都落在 `2026-03-02`。
  3. **主收口**：明确当前可验证样例是 `2026030200001-4`，用户指定的 `北京七十二店 / 58 号桌 / 19:00` 仍需补齐真实数据源后再做最终验证。
- **影响页面**：[[articles/2026-04-14-feishu-stream-pull-validation-plan]]、[[log]]。

### 纠正样例里的订单号表述

- **记录人**：sunhao
- **用户意图**：纠正前一轮文档里关于样例字段的表述，避免把订单号误写成原始业务字段。
- **主题**：
  1. 收紧样例说明里的订单号表述。
  2. 明确订单号和桌号、时间点的关系。
- **关键动作**：
  1. **主修正**：把 [[articles/2026-04-14-feishu-stream-pull-validation-plan]] 里的样例说明改成“订单号只是回溯和定位样例的键，不是原始业务字段”。
  2. **主澄清**：明确真正要核对的是桌号、时间点和门店，订单号只是用来反推对应记录。
- **影响页面**：[[articles/2026-04-14-feishu-stream-pull-validation-plan]]、[[log]]。
