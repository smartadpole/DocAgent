...
- 想知道“新增知识怎么形成网状关联、避免孤岛页” -> 读 [[knowledge-linking-rules]]
...
- 物理上，治理页统一收进 `governance/`，只保留 [[AGENTS]] 在根目录做 agent 规则正文；`CLAUDE.md` 是 Claude Code 适配壳，不承接第二份规则正文；`.codex/AGENTS.md` 如存在只能是 thin Codex adapter，不放项目级规则副本
