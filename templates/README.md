...
这里放系统治理层面的可复制笔记模板：新页面、新事项、新报告、新契约或 agent 交付物需要稳定字段和章节时，才进入这个目录。

不要把所有“模板化”结果都放进这里。知识库模板属于专题成果，例如某个概念下的方法框架、报告结构、分析维度或研究输出骨架，优先写在 owning topic、`articles/`、`concepts/` 或项目专题页中。只有当它需要作为跨项目、跨页面类型的可复制治理骨架时，才升级为本目录模板。
