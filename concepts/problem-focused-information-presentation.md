...
- 首屏面向用户价值，先呈现当前判断、关键风险、下一步、可执行 / 条件性 / 禁止上推和最重要证据；`lens_id`、导出策略、刷新策略等维护字段默认下沉到底部、折叠区、脚注或 registry。
- 照片和视觉证据要先按用户扫读路径、信息价值、行动用途和画幅家族排版；证据图默认不裁切，HTML / print view 优先使用 `object-fit: contain`，避免用固定高度格子制造空白或裁掉关键信息。
...
- 把 metadata 表放在首屏主体，让用户先读维护字段而不是先看到当前判断、风险和下一步。
- 批量照片按上传顺序平铺，或用 `object-fit: cover` 裁掉证据图的关键边缘信息。
