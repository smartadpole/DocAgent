...
| 治理能力 / 协作契约 | Goal Contract、执行合同语义、状态约束推演 | 可迁移的是协作契约和裁决边界；不能把它误写成普通业务 skill，也不能替代验收关闭。Goal Contract 的主正文在模板层，skill 只引用它。 |
...
跨工程技能矩阵里的 `领先` 不是“分数最高”或“多个工程都很优秀”。它必须满足一个更强条件：该工程覆盖了同一技能下其他工程已经出现的独特成熟信号。

自动化当前用可检测的证据信号近似“成熟信息”，包括 `SKILL.md`、`TRANSFER.md`、sensor、views、governance / rules、template、Goal Contract 标记和正文体量等。一个工程只有在得分处于最高档、并且证据信号指纹覆盖同一技能所有工程的信号并集时，才可标为 `领先`。

如果两个工程都很强，但各有独特信号，例如一个有完整 `TRANSFER.md`，另一个有 sensor / views / template 闭环，那么它们都不能同时叫 `领先`；它们是互补优秀样本，应先进入“成熟 / 互补对齐”状态。诊断报告应提示各自缺少哪些对方独特信号，直到某个工程吸收并覆盖全体信号后，才重新判为领先。

这个规则保护两个边界：

- 避免把“并列高分”误读成“并列最优”，导致读者忽略对方的独特实践。
- 鼓励反哺时先合并优秀样本的差异，再把完整方法沉淀回源技能、`TRANSFER.md`、sensor 或示例产物。
