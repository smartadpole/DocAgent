...
AI 时代信息呈现方式，是指在 LLM、agent、RAG 和可交互前端共同参与后，信息从“静态给人阅读”转向“同时服务检索、推理、协作、操作和决策”的组织方式。

核心判断是：信息不再只有一个最终版页面，而应拆成源、索引、关系和界面四层。

## 分层

| 层级 | 典型形态 | 主要职责 |
| --- | --- | --- |
| 源 | Markdown、CSV、代码、原始资料 | 保存事实、版本、来源和审计路径 |
| 索引 | chunk、embedding、vector store、搜索索引 | 帮助模型和人快速召回候选内容 |
| 关系 | wikilink、backlink、目录、上位概念、邻接页面 | 让知识结构、依赖、冲突和演进显式可见 |
| 界面 | HTML、Artifact、Notebook、dashboard、slide deck | 支持实时呈现、筛选、比较、复现和决策 |

## 基本原则

- Markdown 更适合作为长期真相源；HTML 更适合作为交互呈现层。
- 向量库是派生索引，不是知识主库。
...
- 设计从资料收集到实时呈现的工作流。
- 把静态文档升级成可交互 dashboard、Notebook、Artifact 或 HTML report。
