...
- `BRAIN.md` 是共享脑，承接跨多轮确认、会持续影响后续工作的共同背景。
...
- 只反映时间先后的过程记录，进入 `log.md`，但 `log.md` 不是主动背景。
...
- 项目运行层是 `projects/README.md` 加上需求、设计、决策、开发、发布、事故这些页面，回答“当前项目正在做什么、为什么这样做、做到哪里了”。
...
- 如果目标在 `projects/`，先读 `projects/README.md` 和 `projects/STRUCTURE.md`，再读相关的需求、设计、决策、开发页面。
...
- 如果新需求和 `BRAIN.md`、`AGENTS.md`、`projects/STRUCTURE.md` 或既有项目决策发生碰撞，不要直接覆盖旧结论。
- 在项目阶段，先升级到 `projects/decisions.md`，形成明确决策后，再同步更新相关页面。
- 没有形成决策前，不把冲突内容直接写入共享脑。
...
- 仓库内文档之间的引用，默认使用可迁移的相对关系或 `wikilink`，避免绑定到某一台电脑的目录结构。
