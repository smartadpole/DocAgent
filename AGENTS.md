...
- 升级 agent 规则、治理层、memory、workflow、skill、harness、evaluator、sensor、template 或 agent 入口默认行为时，先判 `upstream_write_authorization`：只有用户或 Run Capsule / dispatch 明确授权上游写入，且 allowed write roots 包含 AcknowledgeBase owner 时，才可受控直编 AcknowledgeBase；本仓普通运行即使本机能访问 AcknowledgeBase，也只能保留本仓库项目事实、handoff、验证证据和 conformance，并产出 system-layer extraction / handback。handback 必须写清 owner discovery、system-layer delta、本地 conformance、验证证据、禁止项、不上推边界，以及 `design-owner: existing / deferred / not-applicable`；不得把本仓事实写进 AcknowledgeBase 或用户级 `~/.codex/AGENTS.md`。
