...
const DEFAULT_SETTINGS = {
  ...
  enableInterlinearText: true,
  interlinearBatchSize: 4,
...
class TooltipManager {
  ...
  showPlain(text, rect) {
    ...
    el.classList.remove('mtt-tooltip-interlinear');
  ...
  _splitInterlinearChunks(text) {
    ...
    for (let i = 0; i < source.length; i++) {
      ...
      buf += ch;
      const numericComma = ch === ',' && /\d/.test(prev) && /\d/.test(next);
      const hardBoundary = /[;:.!?]/.test(ch) && (!next || /\s/.test(next));
      const longCommaBoundary = ch === ',' && !numericComma && buf.length >= 120 && (!next || /\s/.test(next));
      if (hardBoundary || longCommaBoundary) {
    ...
    const clauseStarters = new Set([
      'that', 'which', 'who', 'whom', 'whose', 'because', 'although', 'though',
      'while', 'when', 'where', 'if', 'unless', 'until', 'since', 'before',
      'after', 'so', 'but', 'and', 'only',
    ]);
    const predicateStarters = new Set(['is', 'are', 'was', 'were', 'has', 'have', 'had', 'can', 'could', 'should', 'would', 'will']);
    const connectorStarters = new Set(['and', 'or', 'but']);
    const pushReadableUnits = (part) => {
      const words = part.split(/\s+/).filter(Boolean);
      if (words.length <= 9 && part.length <= 78) {
        pieces.push(part);
        return;
      }
      let local = [];
      for (let i = 0; i < words.length; i++) {
        const word = words[i];
        local.push(word);
        const phrase = local.join(' ');
        const remaining = words.length - i - 1;
        const next = (words[i + 1] || '').toLowerCase().replace(/^[("']+|[),.;:!?'"`]+$/g, '');
        const punctBoundary = /[,;:)]$/.test(word) || /[\u2014\u2013-]$/.test(word);
        const clauseBoundary = clauseStarters.has(next) && local.length >= 5 && remaining >= 4;
        const predicateBoundary = predicateStarters.has(next) && local.length >= 7 && remaining >= 4;
        const connectorBoundary = connectorStarters.has(next) && local.length >= 6 && remaining >= 4 && /[,;:]$/.test(word);
        const maxBoundary = local.length >= 10 || phrase.length >= 78;
        const softBoundary = local.length >= 5 && remaining >= 4 && punctBoundary;
        if (maxBoundary || softBoundary || clauseBoundary || predicateBoundary || connectorBoundary) {
          pieces.push(phrase);
          local = [];
        }
      }
    ...
    for (const part of rawParts) {
      if (!part) continue;
      pushReadableUnits(part);
    }

    const chunks = [];
    for (let i = 0; i < pieces.length; i++) {
      let piece = pieces[i];
      if (/^\d+[.)]?$/.test(piece) && i + 1 < pieces.length) {
        pieces[i + 1] = `${piece} ${pieces[i + 1]}`;
        continue;
      }
      const wordCount = countWords(piece);
      if (chunks.length && (wordCount <= 2 || piece.length < 16 || /^[/),;:]/.test(piece))) {
  ...
  async _translateInterlinearChunk(chunk, engine, sourceLang, targetLang) {
    const eng = ENGINES[engine] || ENGINES.google;
    const key = `v2-interlinear|${engine}|${sourceLang}|${targetLang}|${chunk}`;
    if (!this.plugin.settings.disableCache && this.interlinearCache.has(key)) {
      return this.interlinearCache.get(key);
    }
    try {
      const result = await eng.translate(chunk, sourceLang, targetLang, this.plugin.settings);
      if (result?.targetText && !isNoopTranslation(result, chunk, this.plugin.settings)) {
        const row = { source: chunk, target: result.targetText };
        if (!this.plugin.settings.disableCache) this.interlinearCache.set(key, row);
        return row;
      }
    } catch (_) {
  ...
  async _translateInterlinearChunks(chunks, engine, sourceLang, targetLang, onProgress) {
    const batchSize = Math.max(1, Number(this.plugin.settings.interlinearBatchSize || 4));
    const rows = chunks.map(source => ({ source, target: '' }));
    for (let i = 0; i < chunks.length; i += batchSize) {
      const batch = chunks.slice(i, i + batchSize);
      const translated = await Promise.all(batch.map(async (chunk, offset) => ({
        index: i + offset,
        row: await this._translateInterlinearChunk(chunk, engine, sourceLang, targetLang),
      })));
      for (const { index, row } of translated) rows[index] = row;
      if (onProgress) onProgress(rows.slice(), i + batch.length >= chunks.length);
    }
    return rows;
  }
  _appendInterlinear(el, chunks) {
    el.classList.add('mtt-tooltip-interlinear');
    const wrap = document.createElement('div');
    wrap.className = 'mtt-interlinear';
    el.appendChild(wrap);
    this._renderInterlinear(wrap, chunks.map(source => ({ source, target: '' })), false);
    return wrap;
  }
  _renderInterlinear(wrap, rows, done) {
    if (!wrap) return;
    wrap.empty ? wrap.empty() : (wrap.textContent = '');
    const label = document.createElement('div');
    label.className = 'mtt-interlinear-label';
    label.textContent = '对照';
    wrap.appendChild(label);
    for (const row of rows) {
      const pair = document.createElement('div');
      pair.className = 'mtt-interlinear-pair';
      if (`${row.source || ''} ${row.target || ''}`.length > 96) {
        pair.className += ' mtt-interlinear-pair-wide';
      }
      const target = document.createElement('div');
      target.className = row.target ? 'mtt-interlinear-target' : 'mtt-interlinear-target mtt-interlinear-pending';
      target.textContent = row.target || (done ? '翻译失败' : '翻译中...');
      const source = document.createElement('div');
      source.className = 'mtt-interlinear-source';
  ...
  async show(text, rect, engineKey) {
    ...
    if (!showDict && this.plugin.settings.enableInterlinearText && engine === this.plugin.settings.selectionEngine) {
      const chunks = this._splitInterlinearChunks(text);
      if (chunks.length > 1) {
        const interlinear = this._appendInterlinear(el, chunks);
        this._translateInterlinearChunks(chunks, engine, sourceLang, targetLang, (rows, done) => {
          if (my !== this.token || !this.el || this.el.style.display === 'none') return;
          this._renderInterlinear(interlinear, rows, done);
          this.position(rect);
        })
          .then((rows) => {
            if (my !== this.token || !this.el || this.el.style.display === 'none') return;
            this._renderInterlinear(interlinear, rows, true);
            this.position(rect);
          })
          .catch(() => {
            if (my !== this.token || !interlinear) return;
            this._renderInterlinear(interlinear, chunks.map(source => ({ source, target: '' })), true);
            this.position(rect);
          });
      }
    }
    if (this.plugin.settings.showSourceText) this._appendSourceText(el, text);
