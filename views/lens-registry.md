...
The entries below are retained for local path / lens_id compatibility under `scripts/fixtures/topic-visual-presentation/legacy_artifact_baseline_manifest.v1.json`; they are not public pages, canonical runtime entries or aliases.
...
- `evidence_boundary`: confirmed for packed owner Markdown rendering, browser-parseable JavaScript and current local HTML link rewriting; source pages remain Markdown truth sources; no public publication is configured.
...
- `refresh_trigger`: current HTML links to Markdown owner pages, linked owner Markdown or viewer script changes.
