...
| `projects/design/topics/problem-focused-lens-default-invocation-boundary.md` | 问题聚焦 Lens 是默认判断，不是默认生成持久页面。 | skill / workflow / evaluation / template / topic | [[skills/problem-focused-visual-presentation/SKILL]]、[[problem-focused-visual-presentation-rules]]、[[templates/problem-focused-lens-template]] | 用户要求直观、图文或复杂状态时进入 lens 判断；是否落 HTML 由 source pack 和 publication gate 决定。 | `problem-focused-visual-presentation`、`public-html-publish`。 |
| `projects/design/topics/research-operating-system-design.md` | 调研从搜索升级为 Research Contract、Source Ledger、Evidence Reading、Taxonomy、Judgment、Assets、Capability Upgrade 和 Sensor。 | skill / workflow / memory / evaluation / template | [[skills/research-capability/SKILL]]、[[skills/technology-research/SKILL]]、[[templates/research-intake-template]]、[[templates/technology-research-evidence-matrix-template]] | 深度调研必须先定研究等级、源计划、证据矩阵和行动等级；采购 / 产品类补视觉证据 lane。 | `research-capability`。 |
...
| `projects/design/topics/technical-research-capability-upgrade.md` | 技术研究需要 R2+ Source Plan、coverage matrix、Research Case Packet 和 case index。 | skill / workflow / evaluation / template / memory | [[skills/technology-research/SKILL]]、[[templates/technology-research-contract-template]]、[[templates/technology-research-report-template]]、[[research-capability-rules]] | 系统方案、性能工程和开源技术调研必须先做 source plan checkpoint 与 coverage matrix。 | `research-capability`。 |
