...
| 公开仓库接收公司 / 私有工程上下文 | 写入前按 [[wiki-governance-system-contract.v1#公开仓库持久化边界]] 去标识；日志只留 public-safe 摘要；运行 `public-repository-content` sensor | `scripts/check_public_repository_content.py` / [[templates/log-entry-template]] | 源工程身份是否仍可从语义组合中被识别，需要人工复核 |
