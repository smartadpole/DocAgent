...
| 引导式设计 | “设计一个系统”“帮我想完整”“更智能 / 更前沿 / 更高效”“还不知道怎么开始”；新 clone / 模板工程中的“你好 / hi / 开始吧” | 自动判定场景包、置信度和关键假设；首次接触必须给 onboarding 形态：可见正文逐项输出系统角色、用户目标、协作方式、第一步成果，并给推荐第一步；不能输出“梳理当前项目状态 / 检查当前项目状态 / 写入规则 / 实现 / 验收 / 收尾 / 提交”这类使用过程菜单 | [[proactive-dialogue-system]]；首次接触是零工具 / 零读盘 / 零命令路径，只用已加载上下文和启动入口身份，身份不确定时先给“定位工程身份”选项；不得主动读取运行层状态 | 条件写入；首次接触完成最低回复但没有新增判断可 no-op，形成项目意图 / 体系校准时必须产物化 | 写清场景包、假设、待确认项、性能预算和落地位置；no-op 时说明没有新增可沉淀信息 |
