...
1. 入口层：[[README]]、[[INDEX]]
2. 治理层：[[governance/README]]、[[AGENTS]]、[[WORKFLOW]]、[[response-mode-routing]]、[[instruction-adherence]]、[[execution-contract-semantics]]、[[harness-evolution]]、[[harness-feedback-ledger]]、[[POLICY]]、[[BRAIN]]
...
5. 如果要判断规则已有但没执行的问题，再看 [[instruction-adherence]]
6. 如果要判断执行页是否出现口径漂移，再看 [[execution-contract-semantics]]
7. 如果要判断本轮偏差是否应沉淀成 episode，再看 [[harness-evolution]] 和 [[harness-feedback-ledger]]
8. 如果要实际执行修改，再看 [[WORKFLOW]]
9. 如果要知道 agent 的行为边界，再看 [[AGENTS]]
10. 如果要治理 `[[log]]`、[[projects/trace]] 或模板反哺，再看对应规则页
