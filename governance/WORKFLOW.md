...
长时任务先补 Goal Contract：

- 主控侧任务如果要“持续推进直到完成”，先用 [[templates/goal-contract-template]] 定义期望最终状态、验证面、约束、边界、迭代策略和阻塞停止条件。
- 子工程接到主控任务时，不重新定义关闭口径；只把 Goal Contract 转成实现范围、测试方案、回传包和未验证边界。
- Goal Contract 可以作为 `/goal` 草稿，但它本身不是验收结论；状态关闭仍按验收关闭模式执行。
