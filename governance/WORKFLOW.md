...
8. 收尾前做一次二阶反思：判断本轮问题是否暴露同类漏检、流程缺口、模板缺口、协作契约缺口或记忆路由缺口；如果是，就按 [[POLICY]] 回写对应规则、流程、模板、共享背景或 [[log]]。
...
- 主题图文呈现进入 `views/`，只承接 current / snapshot 派生视图，不替代事实源；admit 后固定 subject/source、三轴、五门 evaluator 和版本化 trace，并同轮生成同源 PDF / PNG 到忽略目录
...
- `views/`：[[README]]、[[INDEX]]、[[views/README]]、[[skills/topic-visual-presentation/SKILL]]、[[templates/topic-presentation-template]]、目标事实源和 [[views/lens-registry]]
...
4. 把候选变化分成系统规则、系统结构、系统流程、系统模板、系统记忆路由、自动化契约和项目材料。
5. 对反哺项做脱敏和抽象，删掉项目名、业务名、具体数据、具体状态、具体技术拍板和一次性会议内容。
6. 只要候选项是规则，默认进入反哺候选；如果决定不反哺，必须能说明它为什么只是项目材料或为什么暂不具备通用性。
7. 写入前先做抽象、事实剥离、冲突、单一信息源和规则体积检查；默认先改旧规则，旧规则容纳不下时才新增。
8. 按路由更新模板：结构进 [[projects/STRUCTURE]] 和入口页，流程进 [[WORKFLOW]]，硬约束进 [[AGENTS]]，规则和记忆路由进 [[POLICY]]，共享背景进 [[BRAIN]]，可复制写法和模板骨架进 `templates/`。
