...
每轮开始时，先按 [[response-mode-routing]] 判断当前响应模式，再决定读取深度、默认写入和收尾动作。

如果本轮是在纠正 agent 治理过重、规则过硬、入口过长，或发现 `[[log]]`、产物化、完整检查、二阶反思、Goal Contract、模板反哺被当成无条件仪式，先读 [[agent-governance-strategy]]，把动作分成 P0 硬约束、P1 语义门、P2 流程和 P3 backlog。
...
1. 先判断响应模式，再判断这次问题是简单更新，还是复杂问题。
2. 如果是复杂问题，先拆成几个中间节点，再逐个完成，不要一次改一大坨。
3. 开始改之前，先建立全局背景，再找这次问题的主入口、单一信息源和相关跳转页。
4. 更新时优先改主页面，其他页面只补链接、简短说明或导航，不复制正文。
5. 如果新增长期知识页，按 [[knowledge-linking-rules]] 检查入口、上位概念、邻接页面和反向承接，不让新页只靠 [[log]] 被发现。
6. 每个中间节点完成后，检查结构、链接、阶段映射和职责是否仍然清楚。
7. 复杂问题在中间节点完成后就提交；只要这次对话产生了实际内容变更或结构变更，保底在对话结束前提交一次。
8. 只要这次对话产生了实际内容变更或结构变更，就做 log eligibility 判断；影响规则、结构、状态、决策、验收、跨工程边界或长期知识时写入 `[[log]]`，纯格式、错别字、无语义链接修复和临时本地状态可免写。
9. 收尾前做一次二阶反思资格判断：本轮问题是否暴露同类漏检、流程缺口、模板缺口、协作契约缺口或记忆路由缺口；如果是重复失守、P0 风险或可复用教训，就按 [[POLICY]] 和 [[agent-governance-strategy]] 回写对应规则、流程、模板、共享背景或 [[log]]。
10. 如果这轮是在对一份新方案或新架构稿逐项拍板，拍板后不能只留下决策结论；必须把与该项决策直接相关的原稿信息同步沉淀到 `projects/design/` 的当前主设计页或设计储备页，避免后续重复回读原稿。
...
- 测试计划和验收合同：看 `projects/development/plan/test-acceptance-planning-model.md`
- AP 验收计划：看 `projects/development/acceptance/README.md` 和 `projects/development/acceptance/plans/README.md`
...
- [[instruction-adherence]]：写已有规则如何被触发、检查和最终证明。
- [[execution-contract-semantics]]：写执行合同单值裁决、非目标和证据层级防漂移规则。
...
6. 做一次二阶反思：判断这轮是否应该抽象出更高层规则、模板、读取顺序、协作边界或自动沉淀边界；已有规则没执行时，优先把规则改得更可检查，而不是新增平行规则。
7. 提交当前主题改动，不把无关脏改动一起打包。
...
- `projects/development/plan/test-acceptance-planning-model.md`：`projects/development/plan/work-item-system-model.md`、`projects/development/plan/task-design-model.md`、`projects/development/acceptance/README.md`、`projects/development/reports/README.md`
...
- `projects/development/acceptance/README.md`：`projects/development/plan/test-acceptance-planning-model.md`、相关 EP / TASK、Issue、功能点、Gate、报告入口和服务台账
- `projects/development/reports/README.md`：相关 EP / TASK、Issue、功能点、Gate、AP、worklog、服务台账和验证证据
...
4. 检查它是否需要可复制模板：只要它会反复产生同类记录、任务、报告、风险、回传包或实体页，就必须同步补 `templates/` 模板和 [[templates/README]] 入口；项目页只链接模板，不维护第二份模板正文。
...
3. 如果来源项目提供“不适合吸收”的清单，把它作为反向证据来下游处理项目材料边界，不把它改写成模板到项目的吸收流程。
4. 把候选变化分成系统规则、系统结构、系统流程、知识库模板、系统治理模板、系统记忆路由、自动化契约和项目材料。
5. 对反哺项做脱敏和抽象，删掉项目名、业务名、具体数据、具体状态、具体技术拍板和一次性会议内容。
6. 只要候选项是规则，默认进入反哺候选；如果决定不反哺，必须能说明它为什么只是项目材料或为什么暂不具备通用性。
7. 写入前先做模板分类、抽象、事实剥离、冲突、单一信息源和规则体积检查；默认先改旧规则，旧规则容纳不下时才新增。
8. 按路由更新模板：结构进 [[projects/STRUCTURE]] 和入口页，流程进 [[WORKFLOW]]，硬约束进 [[AGENTS]]，规则和记忆路由进 [[POLICY]]，共享背景进 [[BRAIN]]，知识库模板进 owning topic，系统治理可复制骨架进 `templates/`。
9. 如果反哺新增模块、目录、入口页或高频文件类型，同轮补齐 README、入口链接、上下游关系、最小读取集和 [[log]]。
10. 交付前检查没有项目专名泄漏、没有缺失 wikilink、没有复制第二份模板正文，也没有把同一规则复制到多个主入口。

判断是否可以反哺：

- 规则、流程、结构、记忆路由、写法、模板和自动化契约，去掉项目事实后仍然能跨项目成立，先进入反哺判断；其中模板必须先区分知识库模板和系统治理模板。
- 规则默认进入反哺候选；规则里夹带项目事实时，只抽象规则，不复制事实。
- 知识库模板属于专题成果，优先写 owning topic；系统治理模板属于 `templates/`，必须是可复制页面骨架。
- 只是下游项目的业务决策、技术选择、会议结论、状态推进或实现结果，留在下游项目。
- 后续项目证明规则不通用时，收窄适用范围、降级为项目内规则或退役，不让模板只增不减。
...
5. 如果改动影响结构、入口、约定、状态、决策、验收或长期知识，把变更写进 [[log]]，先写主题和用户意图，再写动作和影响；如果只是纯格式、错别字或无语义链接修复，按 [[agent-governance-strategy]] 标为免写。
...
- 写 `[[log]]` 前先做 log eligibility：证明这轮是否需要记录，而不是机械新增一条记录。
