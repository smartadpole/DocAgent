...
7. 只要这次对话产生了实际内容变更或结构变更，就做 log eligibility 判断；影响规则、结构、状态、决策、验收、跨工程边界或长期知识时写入 `[[log]]`，纯格式、错别字、无语义链接修复和临时本地状态可免写。
8. 收尾前做一次二阶反思资格判断：本轮问题是否暴露同类漏检、流程缺口、模板缺口、协作契约缺口或记忆路由缺口；如果是重复失守、P0 风险或可复用教训，就按 [[POLICY]] 和 [[agent-governance-strategy]] 回写对应规则、流程、模板、共享背景或 [[log]]。
9. 如果这轮是在对一份新方案或新架构稿逐项拍板，拍板后不能只留下决策结论；必须把与该项决策直接相关的原稿信息同步沉淀到 `projects/design/` 的当前主设计页或设计储备页，避免后续重复回读原稿。
