...
5. 只要候选项是规则，默认进入反哺候选；如果决定不反哺，必须能说明它为什么只是项目材料或为什么暂不具备通用性。
6. 写入前先做抽象、事实剥离、冲突、单一信息源和规则体积检查；默认先改旧规则，旧规则容纳不下时才新增。
7. 按路由更新模板：结构进 [[projects/STRUCTURE]] 和入口页，流程进 [[WORKFLOW]]，硬约束进 [[AGENTS]]，规则和记忆路由进 [[POLICY]]，共享背景进 [[BRAIN]]，可复制写法和模板骨架进 `templates/`。
8. 如果反哺新增模块、目录、入口页或高频文件类型，同轮补齐 README、入口链接、上下游关系、最小读取集和 [[log]]。
9. 交付前检查没有项目专名泄漏、没有缺失 wikilink、没有复制第二份模板正文，也没有把同一规则复制到多个主入口。
...
当某个具体项目要吸收这套模板库的更新时，也按 [[template-feedback-rules]] 先做边界判断。

默认顺序：

1. 先确认目标项目已吸收到哪个上游 commit 或版本；本轮只比较该 commit 之后的新变化。
2. 把上游变化分成系统结构、系统规则、系统流程、系统模板、协作契约和模板示例 / 项目事实。
3. 只吸收结构、规则、流程、模板骨架和协作契约；不吸收上游模板库的占位项目、示例 FP / TODO / Gate / 报告 / 风险、示例执行数据或其他项目技术拍板。
4. 如果目标项目只需要最小结构，就只创建入口和空骨架，不搬历史报告、示例实体页或具体 Gate 方案。
5. 默认保持实现工程只读上下文和回传证据模式；除非目标项目明确授权，不开放实现工程直接改主控文档状态、TODO、设计或 trace。
6. 二阶反思只作为收尾检查动作；如果要新增高优先级规则、改变自动写入边界或覆盖旧规则，按 [[POLICY]] 升级。
7. 在目标项目的 [[log]] 或项目记忆里记录本轮吸收的上游 commit / 版本、吸收项和不吸收项，避免下一轮重复判断。
