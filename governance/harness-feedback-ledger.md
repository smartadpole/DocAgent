...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-06-12 | 矩阵主对象被摘要、折叠和重复标签稀释 | 用户连续纠偏技能矩阵：完整能力矩阵不应消失，主次分层不能重复两张矩阵，PDF 必须保留色彩语义，排版要紧凑，全量同类行不应逐行写“优先” | 图文呈现技能升级 + Harness 反馈 | 用户可见理解成本 | 更新 [[skills/problem-focused-visual-presentation/SKILL]] 和 `TRANSFER.md`，把主矩阵不可被摘要 / 折叠替代、同矩阵不重复渲染、标签必须有区分力、矩阵密度质量和详情页承接原则抽象为可迁移规则 | [[skills/problem-focused-visual-presentation/SKILL]] / `skills/problem-focused-visual-presentation/TRANSFER.md` | promoted |
| 2026-06-12 | 可外发教程长对话由用户连续纠偏驱动 | 用户指出 Codex Goal 公开教程对话过长，说明 agent 建设不完善；本轮连续出现文案专业性、案例入口、内部处理词、第二版本受众等用户纠偏 | Agent 工作复盘 + Harness 反馈 | 可优化成本 | 新增 [[projects/retrospectives/2026-06-12-codex-goal-guide-agent-collaboration]]，记录公开教程预检缺口、用户协作风格背景、潜在 public-copy lint 和多轮纠偏后暂停重定向候选；同步 [[projects/memory/shared]] | 复盘档案 / [[projects/memory/shared]] / 后续候选：公开教程预检清单、public-copy lint、连续纠偏重定向触发器 | observed |
