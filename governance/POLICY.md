...
- 快速诊断模式默认不写项目状态、不关闭 TODO / FP / Gate、不提交规则变更、不把候选结论写成验收完成；若诊断结果影响项目状态、长期事实、验收或规则，必须显式切换到对应模式。
- 为了提速而削弱用户可见症状保真、证据分层、权限边界、非默认值验证、人工确认边界或提交闭环，属于无效优化。
- Goal Contract 可以自动写入任务简报、handoff、Harness 接入模板或 episode 模板，用来定义长时任务完成条件；但它不自动关闭 TODO / FP / Gate / release，也不替代本轮独立验收。
...
- agent 可以自动写入 `skills/` 下的项目内技能页，前提是该技能只承接可复用分析流程、判断框架、输出格式和回写守卫；技能页不得替代项目状态、需求、设计、TODO、测试报告或服务台账的单一信息源。
...
- 如果发现 owner、凭据、真实环境、真实业务结果、UI 截图、阶段准出、外部系统事实或业务拍板仍需人工确认，必须同轮列成 `待人工确认项` 并同步到待办、风险或会议归口。未确认前，相关 TODO / FP / Gate 只能停在 `review`、`blocked` 或待定状态。
