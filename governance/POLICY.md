...
- agent 可以自动写入 `views/` 下的 current / snapshot lens，前提是用户要求持久图文呈现、HTML、PDF / PNG、状态页、风险页、计划页、验收页、决策页或本轮形成稳定可复看视图；lens 必须回链 source pack、声明证据边界、同步 registry、保留 `static_visual_qa`，并把导出件放入忽略目录。Lens 不得替代项目状态、验收关闭、Issue 关闭、决策拍板、测试报告、复盘档案或规则裁定。
