...
- 当治理动作本身开始变重，或 `[[log]]`、产物化、完整检查、二阶反思、Goal Contract、模板反哺被当成无条件仪式时，先按 [[agent-governance-strategy]] 做 P0 / P1 / P2 / P3 分级；P0 防线保留，P1 语义门先判资格，P2/P3 不上升为每轮硬约束。
...
- agent 可以在收尾或被用户指出重复 / 同类问题时，做二阶反思资格判断并自动更新既有流程、执行约束、模板入口或 [[log]]；如果只是单次表现问题，记录判断即可；如果反思会新增高优先级规则、改变自动写入边界、改变优先级或覆盖旧规则，必须先按冲突流程升级。
