...
- agent 可以自动写入 [[projects/retrospectives/README]] 和具体复盘档案，前提是本轮已经形成阶段、专题、交付链、Issue / 事故后或 Agent 协作的长期学习价值。复盘不能替代 [[log]]、Issue、事故、测试报告、决策、memory 或 trace；复盘行动项必须分流到已有 owner 页面。
...
- 复盘结论按影响分流：单次表现写复盘或 [[log]] 继续观察；重复失守进 [[harness-feedback-ledger]]；可模板化进 `templates/`；可技能化进 `skills/`；可脚本化进 sensor；影响执行顺序进 [[WORKFLOW]]；影响必须 / 禁止行为进 [[AGENTS]]；影响优先级或自动沉淀边界才进 [[POLICY]]。
