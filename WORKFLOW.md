...
- `[[log]]` 的合并与拆解优先看用户意图，而不是问题个数、修改文件个数或日期是否相同。
...
- 跨日期的同主题如果只是承接旧背景，按“延续”处理，在今天的新记录里说明承接关系，不改旧正文。
- 跨日期的同主题如果已经形成新的独立用户意图、独立主结果或独立判断任务，按“重开”处理，直接写成今天的新记录。
- 只有在旧记录归属日期写错，或旧记录漏掉了本应属于当时那轮对话的内容时，才按“回填”处理并回改旧日期。
- 不机械转录用户每一句提问，而是先下游处理这次对话真正围绕的主题。
