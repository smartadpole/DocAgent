...
这部分回答的是：当后续开始用 AI 真正研发一个具体功能时，怎么从需求一路推进到上线，而不是只停留在写文档。
...
1. `功能定义`
   先读 `projects/README.md`，确认这个功能当前属于什么阶段、目标是什么、和当前项目方向是否一致。
   如果项目主页里还没有这个功能，就先补一条最小说明：问题、目标、范围、下一步。

2. `需求确认`
   如果需求边界不清、验收标准不清、存在多种理解，就补 `requirements.md`。
   至少写清：用户问题、目标、范围、非目标、验收标准。

3. `方案确认`
   如果功能会引入新模块、新接口、新数据流、较大重构或明显技术取舍，就补 `design.md`。
   如果只是局部小改、实现路径很直接，可以跳过正式设计页，直接进入实现。

4. `关键决策记录`
   如果实现过程中出现“为什么这样做而不是那样做”的关键判断，就写进 `decisions.md`。
   这一步不是每次都要单独成文，但关键取舍不能只留在聊天里。

5. `实施拆分`
   功能复杂时，先拆成 2 到 5 个中间节点，每个节点都要有：
   - 明确目标
   - 明确验证方式
   - 明确 commit 边界
   小功能可以一个节点做完，大功能不要一次性改到底。

6. `代码实现`
   AI 在代码仓库里实际修改代码、补测试、跑验证。
   如果过程复杂、排障很多、回滚思路重要，再补 `worklog.md`。
...
8. `发布与回写`
   如果已经接近上线或需要交付说明，就补 `releases.md`。
   如果这次功能产出了可复用结论、通用方法或稳定概念，再回写到 `articles/`、`concepts/`、`indexes/`。

### 1.9.10 功能从需求到上线的最小闭环

最小闭环可以压成下面这条：

1. 在 `projects/README.md` 写功能目标和下一步
2. 必要时补 `requirements.md`
3. 必要时补 `design.md`
4. AI 写代码并验证
5. 关键取舍记到 `decisions.md`
6. 需要时补 `releases.md`
7. 把稳定结论回写知识库
...
- 验收标准不明确
- 方案存在多种可选路径
- 会影响多个模块、接口、数据结构或长期架构
- 这次改动后续大概率要复盘、复用或向别人解释
...
- 改动局部且技术路径直接
- 验证方式清楚
- 不需要额外设计文档也不会造成后续理解困难

补文档和写代码之间，不是二选一，而是按复杂度决定最小前置文档量：

- 小功能：项目主页一条说明 + 直接编码
- 中功能：项目主页 + `requirements.md` 或 `design.md`
- 大功能：项目主页 + 需求 + 设计 + 决策 + 分步实施

### 1.9.12 AI 推进功能时的默认检查表

每次让 AI 推一个具体功能，默认先检查：
