...
- [[log]] 只负责按时间降序呈现本轮主题、用户意图、关键动作和结构变化，不负责主动参与后续思考。
...
- [[log]]：写时间降序的主题化记录，收口用户意图、关键动作和结构调整。
...
- 不机械转录用户每一句提问，而是先下游处理这轮真正的主题。
- 记录用户提问时，优先提炼成“用户想解决什么”的意图，而不是问题清单。
- 同一轮里多个细碎追问，如果都服务同一主题，合并成一条记录。
- 每条记录至少回答三件事：这轮在解决什么、为什么要解决、实际做了哪些关键动作。
