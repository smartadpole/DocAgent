...
2. 如果是，补进 `BRAIN.md`。
3. 如果它只是个人偏好，再写进 `workspace-memory`，不塞进共享脑。
4. 如果它和已有背景冲突，先升级到 `projects/decisions.md`，不要直接改写共享脑。
...
项目层的目录、文件职责、依赖关系和默认读取顺序，统一看 [[projects/STRUCTURE]]。
...
如果项目会持续变化，再按需要补项目层的需求、设计、决策、执行、发布、事故页面。这些都是可选扩展，不是启动项目时的必选项，具体文件落点看 [[projects/STRUCTURE]]。
...
2. `需求确认`
   如果需求边界不清、验收标准不清、存在多种理解，就补需求页。
   至少写清：用户问题、目标、范围、非目标、验收标准。

3. `方案确认`
   如果功能会引入新模块、新接口、新数据流、较大重构或明显技术取舍，就补设计主入口；只有某块内容明显变大时，再拆成架构或数据库等设计子页。
   如果只是局部小改、实现路径很直接，可以跳过正式设计页，直接进入实现。

4. `关键决策记录`
   如果实现过程中出现“为什么这样做而不是那样做”的关键判断，就写进决策页。
