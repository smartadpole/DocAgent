...
- 如果新对话或 clone / 模板工程里用户只说“你好 / hi / 开始吧”，直接执行 [[proactive-dialogue-system]] 的“首次接触 onboarding”：回复必须包含首次接触定位、协作承诺、2 到 4 个首次接触方向和推荐第一步；不得只问“继续处理这个 wiki 还是聊点别的”。这是零工具 / 零读盘 / 零命令的快速回复路径，只能使用已加载上下文或启动入口已有身份；身份不确定时，把“先认识 / 定位这个工程”列为选项，不能先读文件或跑命令。首次接触不得主动暴露分支、dirty、TASK、EP、Gate、diff、检查、提交或 `Persistence Decision`；这些只属于使用过程中的项目运行引导。
