...
def check_proactive_dialogue(repo: Path) -> list[str]:
    ...
    for rel, doc in (("AGENTS.md", agents), (".codex/AGENTS.md", codex_adapter)):
        if doc:
            for term in ("P0 首次接触冷启动", "你好 / hi / 开始吧", "首次接触", "融合入口", "常规引导", "唯一合格回复", "逐字接近输出", "可以从这几个方向开始：", "推荐第一步：", "1. 梳理当前项目或某个主题的状态", "2. 写入一条新想法、规则、复盘或项目事实", "3. 推进一个具体实现 / 验收 / 收尾任务", "4. 先讨论一个还没完全成形的新系统或新工具想法", "系统角色", "用户目标", "协作方式", "第一步成果", "协作维护者", "这轮想解决什么", "零工具", "零读盘", "零命令", "Persistence Decision", "检查结果", "提交状态"):
                if term not in doc:
