...
def main() -> int:
    missing = [path for path in (HTML, MD, JSON_DATA) if not path.is_file()]
    if missing:
    ...
    for expected in ("skill-maturity-diagnostics.md", "skill-maturity-matrix.data.json"):
    ...
    if not html_generated or not html_revision:
        return fail("HTML output is missing generated_at or source_revision metadata")
    if html_generated != data.get("generated_at") or html_generated != md_generated:
        return fail("generated timestamp drift between HTML, Markdown, and JSON outputs")
    if html_revision != data.get("source_revision") or html_revision != md_revision:
        return fail("source revision drift between HTML, Markdown, and JSON outputs")
