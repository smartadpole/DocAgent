...
## Source Archetype Pack

| Source archetype | 本轮用途 | 边界 |
| --- | --- | --- |
| knowledge-governance source | agent-system maturity、per-dialogue / run trace、migration acceptance、scoring boundary | 不复制分数、profile hash、注册表明细或项目事实。 |
| evaluation-platform source | alignment map、Agent Memory Contract、evaluation correction closeout、source freshness | 不复制数据集、benchmark 或工程记录。 |
| runtime-service source | L5 blocked-boundary proof、Run Capsule、Loop Contract、final reply contract | 不复制远程机器事实或 handoff 路径。 |
| production-control source | issue / work-item / lens / publication 的主控治理信号 | 不复制业务链路或服务实例。 |
| product-intelligence source | Frontier Technology Intake 和 Intelligence Contract | 不复制业务候选技术结论。 |
| operations / adapter / orchestration / model / implementation source | 候选记忆、runtime、cross-project audit 信号 | 只保留 source archetype coverage，不记录工程身份。 |
...
| evaluation correction closeout | true-gap-lite | adapt | 报告口径和 future sensor 候选 | 未迁移 source project 的具体记录。 |
