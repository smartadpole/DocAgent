...
- **角色**：agent
- **本轮变化**：
  - **主补强**：把状态与约束推演从“一张总模板”升级成一组按维度拆分的报告资产。
  - **维度拆分**：新增总览、可执行性、依赖传播、时间资源窗口、阻塞不确定五类模板，避免所有计划问题都塞进同一张大表。
  - **模板入口**：把这组资产接入 [[templates/README]] 和 [[state-constraint-reasoning]]，明确按问题重心选模板，而不是临时自由发挥。
- **当前实现口径**：
  - 状态与约束推演现在既有统一总模型，也有多维报告资产族。
  - 后续同类问题优先复用对应资产模板，不再默认临时重写报告结构。
